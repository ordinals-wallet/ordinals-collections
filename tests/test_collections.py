...
def test_home_structure():
    ...
    correct_directories = [
        x in expected_directories for x in current_directories]
...
def test_collections_structure():
    ...
    assert all(
        folders), 'Invalid structure, include your files in a nested directory'
...
def test_meta():
    ...
    for x in current_collections:
        ...
        assert set(meta.keys()) == set(
            expected_meta.keys()), 'Invalid meta data keys'

        for y in zip(meta.values(), meta.keys()):
            assert isinstance(y[0], str), 'Invalid data type, use a string'
            if y[1].endswith('link'):
                if y[0]:
                    assert y[0].startswith(
                        'https://') or y[0].startswith('http://'), 'link must start with https://'

        assert (len(meta.get('inscription_icon')) == 66) or meta.get(
            'inscription_icon'), 'Invalid inscription Id'
        assert meta.get('slug').lower() == meta.get(
...
def test_inscriptions():
    ...
    for x in current_collections:
        ...
        for y in inscriptions:
            ...
            assert y.get('meta')
            assert y.get(
                'attributes') is None, 'Attributes belong in meta object'
            ...
            assert ishex(y.get('id')[0:64]
                         ), 'inscription ids must be valid hex'
